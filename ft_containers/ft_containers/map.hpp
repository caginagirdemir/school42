...
namespace ft {
    ...
    private:
        ...
        std::allocator<bt_node<key_type, mapped_type> > _tree_allocator;
        key_compare     _comp;
        node            _rend;
        node            _root;
        size_type       _length;
    ...
    public:
        ...
        explicit map (const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) {
            ...
            _tree_allocator.construct(_end, bt_node<key_type, mapped_type>(ft::make_pair(key_type(), mapped_type())));
        ...
        ft::pair<iterator,bool> insert (const value_type& val) {
            if (_root == _end) {
                ...
                _tree_allocator.construct(tmp, bt_node<key_type, mapped_type>(val));
                tmp->right = _end;
                _end->parent = tmp;
                tmp->parent = _rend;
                _rend->right = tmp;
            ...
            node cur = _root;
            while (true) {
                if (cur->element.first == val.first)
                    return ft::make_pair(iterator(cur), false);
                else if (_comp(val.first, cur->element.first)) {
                    if (!cur->left) {
                        node tmp = _tree_allocator.allocate(1);
                        _tree_allocator.construct(tmp, bt_node<key_type, mapped_type>(val));
                        cur->left = tmp;
        ...
        void erase (iterator position) {
            ...
            if (n->left) {
                ...
                std::swap(n->element, rightmost->element);
                ...
                if (n->right == _end) {
                    std::swap(n->element, _end->element);
                    n->left = NULL;
                    n->right = NULL;
                    _length--;
                    _tree_allocator.destroy(_end);
...
//             cur = q.front();
//             int k = (m[cur] - spaces) * 3 - count_digit(cur->element.first) + 1;
//             std::cout << std::string(k >= 0 ? k : 0, ' ') << cur->element.first;
